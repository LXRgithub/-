                ...
                <el-aside width="200px">
                    <el-container class="teacher-role-set" v-if="isMenu1">
                        ...
                        <el-main>
                            ...
                            <div class="user-list">
                                <div v-for="(item, index) in customRoleData" :key="index" class="user-role role-pop"
                                    :class="{actClass: actRole == item.id}"
                                    @click="clickRoleList(item.id)">
                                    <el-popover placement="left" trigger="hover" v-model="item.state" popper-class="author-pop">
                                        <!-- <div style="text-align: center; margin: 0"> -->
                                            <el-button  icon="el-icon-edits" @click="clickPopover(item)"></el-button>
                                        <!-- </div> -->
                                        <el-input  slot="reference" :readonly="true" class="pop-input"
                                            v-model="item.name" @blur="blurEditInput(item)"
...
export default {
  ...
  methods:{
    ...
    clickPopover(item){
        item.state = false;
        // item.only = false;
        let refInput = this.$refs[`input${item.id}`][0].$el;
        let input = $(refInput).find('input');
        console.log('edit-input',input)
        // console.log('edit-input',$(refInput).find('input')[0].attr('readOnly','true'))
    ...
    blurEditInput(item){
        ...
        console.log('blurEditInput',input.val())
    },
    showInput() {
