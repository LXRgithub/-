<!-- 管理员角色-档案管理-教职工档案 -->
<template>
  <div class="teach-record">
       <el-container>
            <el-header>
                <el-row>
                    <el-col :sm="12" :md="12" :lg="12" :xl="12">
                        <div class="title">教职工档案</div>
                    </el-col>
                    <el-col :sm="12" :md="12" :lg="12" :xl="12">
                ...
                <el-form class="uip_custom_form" :model="recordForm" ref="changePwdForm" :inline="true">
                    <el-form-item label="部门">
                        <el-select v-model="recordForm.grade" placeholder="请选择" filterable style="width:120px;">
                            <el-option v-for="item in departData" :label="item.name" :value="item.id" :key="item.id"></el-option>
                        </el-select>
                    </el-form-item>
    ...
    getTableData() {
      this.axios("post", "/getStudentRecordData", {}).then(res => {
        ...
        if (res.code == "0") {
          ...
          // 设置页码
          this.tableIndex(this.tableData, 'indexID', this.currentPage, this.pageSize);
        }
      });
    },
    /**
