    ...
    <div class="navmenu-container">
        <div class="navbar" :class="{mgRnull: index === data.length - 1}"
            ...
            <div class="nav-div" @click="item.disable == true? '':navClick(index,item)"
                ...
                <!-- 右侧箭头图标 -->
                <img class="arrow" :src="item.isSubShow && item.submenus ? arrowNH: (over == index? arrowNH: arrowN)" 
                    v-show="expand && data[index].submenus != undefined" 
                    :class="{'arrow-animate': item.isSubShow && item.submenus ? arrowS: (over == index? arrowNH: arrowN)}"> 
            </div>
            <!-- 二级菜单 -->
