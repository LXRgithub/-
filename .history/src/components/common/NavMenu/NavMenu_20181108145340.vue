    ...
    <div class="navmenu-container">
        <div class="navbar" :class="{mgRnull: index === data.length - 1}"
            ...
            <div class="nav-div" @click="item.disable == true? '':navClick(index,item)"
                 ...
                 :class="{actives:(active == (item.routeUrl||index)), overact: over == (item.routeUrl?item.routeUrl:index),
                 menuStyle,disableClass: item.disable == true}">{{active +'--'+ item.routeUrl+index}}
                <!-- 左侧菜单图标 -->
                <span class="icon-div">
...
<script>
	...
	export default {
        ...
        watch:{
            active(){
                console.log('active-watch',this.active)
            }
        },
		methods: {
		...
		created() {
            ...
            //让所有的菜单都关闭
            setTimeout(() => {
            console.log('让所有的菜单都关闭',this.active,this.$route.path)
                this.data.forEach((element,index) => {
                   /*  if(this.$route.path == this.active){
                        if(index == this.active){
                        console.log('关闭二级',element,index)
