    ...
    <div class="navmenu-container">
        <div class="navbar" :class="{mgRnull: index === data.length - 1}"
            ...
            <div class="nav-div" @click="item.disable == true? '':navClick(item.routeUrl,item)"
                ...
                <!-- 左侧菜单图标 -->
                <span class="icon-div">{{active+'--'+item.routeUrl}}
                    <img :src="(active == (item.routeUrl!=undefined?item.routeUrl:index) || over == item.routeUrl) ? (item.iconActive? item.iconActive:item.icon) : item.icon"
