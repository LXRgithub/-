<template>
    <div class="navmenu-container">
        <div class="navbar" :class="{mgRnull: index === menuData.length - 1}"
             :style="blockStyle" v-for="(item,index) in menuData" :key="index"
             :mgSpan="index == menuData.length-1">
            <div class="nav-div" @click="item.disable == true? '':navClick(index,item)"
                ...
                <img class="arrow" :src="item.isSubShow && item.submenus ? arrowS: (over == index? arrowNH: arrowN)" 
                    v-show="expand && menuData[index].submenus != undefined"> 
            </div>
            <!-- 二级菜单 -->
            <div class="sub-menu" v-show="menuData[index].isSubShow">
                <div class="scorll-div" v-for="(sub,subindex) in menuData[index].submenus" :key="subindex"
...
<script>
	...
	export default {
		...
		created() {
            ...
            //让所有的菜单都关闭
            console.log('让所有的菜单都关闭',this.menuData)
			this.menuData.forEach(element => {
                console.log('关闭二级',element,this.active)
				// element.isSubShow = false;
			});
		}
	}
