<template>
    <div class="navmenu-container">
        <div class="navbar" :class="{mgRnull: index === data.length - 1}"
            ...
            <div class="nav-div" @click="item.disable == true? '':navClick(item.routeUrl,item)"
                 @mouseover="mouseoverMenu(index,item)" @mouseout="mouseoutMenu(index,item)"
                 :class="{actives:(active == item.routeUrl), overact: over == index,
                 menuStyle,disableClass: item.disable == true}">
                <!-- 左侧菜单图标 -->
                <span class="icon-div">{{active+'--'+item.routeUrl|+',,'+active == (item.routeUrl?item.routeUrl:index) }}
                    <img :src="(active == (item.routeUrl?item.routeUrl:index) || over == item.routeUrl) ? (item.iconActive? item.iconActive:item.icon) : item.icon"
